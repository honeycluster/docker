#!/bin/bash
# Validation functions for xrpld/rippled
...
# ---------------------------------------------------------------------------
# xrpld/rippled-specific validators
# ---------------------------------------------------------------------------
validate_network() {
    local v="${NETWORK:-${NETWORK_NAME}}"
    [ -z "$v" ] && return 0
    case "$v" in
        MAINNET|TESTNET|DEVNET) return 0 ;;
        *)
            log_warn "NETWORK='$v' is not MAINNET, TESTNET, or DEVNET; network-specific defaults may not apply"
            return 0
            ;;
    esac
}
...
# ---------------------------------------------------------------------------
# Main validation function (xrpld/rippled)
# ---------------------------------------------------------------------------
validate_inputs() {
    ...
    # Ports used by rippled
    validate_port "PORT_PEER"
    validate_port "PORT_RPC"
    validate_port "PORT_WSS"
    validate_port "PORT_GRPC"
    validate_port "PORT_RPC_ADMIN_LOCAL"
    validate_port "PORT_WSS_ADMIN_LOCAL"
    ...
    validate_boolean "SSL_CHAIN_ENABLED"
    validate_boolean "PEER_PRIVATE"
    validate_boolean "NODE_DB_ADVISORY_DELETE"

    # Optional: validator list URL (http/https). SNTP_SERVERS is NTP hostnames (e.g. pool.ntp.org), not URLs — do not validate with validate_url.
    validate_url "VALIDATOR_LIST_SITES"

    # Optional directories (when set)
    validate_directory "RIPPLE_CERTS_DIR" "false"
    validate_directory "NODE_DB_PATH" "false"
    validate_directory "DATABASE_PATH" "false"

    validate_dns_resolver

    # NGINX_BIN: only when set (e.g. OpenResty image reusing this script)
    if [ -n "$NGINX_BIN" ] && [ ! -x "$NGINX_BIN" ]; then
        log_error "NGINX_BIN is set but not executable: $NGINX_BIN"
        VALIDATION_ERRORS=$((VALIDATION_ERRORS + 1))
    fi
