 ...
 * Merge order: common < size < role < verbosity < network < user input.
...
export function resolveXrpldConfig(
  ...
  const verbosity = input.presets?.verbosity ?? 'warning';
  ...
  const layers: ReadonlyArray<Record<string, unknown>> = [
