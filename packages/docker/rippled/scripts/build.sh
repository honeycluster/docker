...
# rippled build per https://github.com/XRPLF/rippled/blob/develop/BUILD.md
...
rippled_src=${RIPPLED_SRC:-/opt/ripple}
...
git clone --single-branch --branch "${git_ref}" https://github.com/XRPLF/rippled.git "${rippled_src}"
cd "${rippled_src}"
...
# Install binary to canonical path
mkdir -p "${rippled_src}/bin"
if [ -f rippled ]; then
  cp rippled "${rippled_src}/bin/rippled"
elif [ -f xrpld ]; then
  cp xrpld "${rippled_src}/bin/rippled"
else
  echo "Binary not found. .build contents:" && ls -la . && exit 1
fi
chmod +x "${rippled_src}/bin/rippled"

# Remove clone and build artifacts; keep only the binary
cd "${rippled_src}"
