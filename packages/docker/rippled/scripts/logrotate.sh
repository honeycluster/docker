#!/bin/bash
# Parse [debug_logfile] from rippled.cfg and update the logrotate config to match

CONFIG_FILE="${CONFIG_FILE:-/opt/ripple/etc/rippled.cfg}"
LOGROTATE_CONF="/etc/logrotate.d/rippled"
DEFAULT_LOG_DIR="/opt/ripple/log"

# Read the [debug_logfile] path from a rendered rippled.cfg
...
# Update the logrotate config with the log directory from rippled.cfg
configure_logrotate() {
