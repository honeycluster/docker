#!/bin/bash
# Startup for rippled (CONFIG_FILE from defaults.sh; use /opt/ripple/etc/rippled.cfg)

BIN="${RIPPLE_BIN:-/opt/ripple/bin/rippled}"
...
start() {
    local config_file="${1:-${CONFIG_FILE:-/opt/ripple/etc/rippled.cfg}}"
    local extra_args="${2:-}"
...
# Function to reload rippled configuration
reload() {
    if ! is_running; then
        log_error "rippled is not running, cannot reload"
        return 1
    fi
    log_error "rippled does not support config reload; restart the container to apply config changes"
    return 1
}
